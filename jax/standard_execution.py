...
# # corresponding to LargerConvNet
# init_random_params, predict = stax.serial(
...
#     stax.Relu,
#     stax.AvgPool((1, 1), (1, 1)),
#     stax.Flatten,
#     stax.Dense(10),
# )

# # corresponding to VGG11
# init_random_params, predict = stax.serial(
#     stax.Conv(64, (3, 3), padding='same'),
#     stax.Relu,
#     stax.MaxPool((2, 2), (2, 2)),
...
def private_grad(params, batch, rng, l2_norm_clip, noise_multiplier,
  ...
  # print("clipped_grads is ", clipped_grads)
...
def main(_):
  ...
  def update(_, i, opt_state, batch):
    ...
    temp = grad(loss)(params, batch)
    return opt_update(i, temp, opt_state)

  # @profile
  @jit
  def private_update(rng, i, opt_state, batch):
    params = get_params(opt_state)
    print("number of trainable parameters", sum(x.size for x in jax.tree_leaves(params)))
  ...
  for epoch in range(1, FLAGS.epochs + 1):
    ...
    for _ in range(num_batches):
      if FLAGS.dpsgd:
        temp_shape_as_image = shape_as_image(*next(batches), dummy_dim=True)
        opt_state = \
            private_update(
    ...
    pr.disable()
    if FLAGS.dpsgd:
      pr.dump_stats(f"Standard_private{FLAGS.noise_multiplier}|{FLAGS.l2_norm_clip}|{FLAGS.batch_size}|CPU.prof")
    else:
      pr.dump_stats(f"public{FLAGS.noise_multiplier}|{FLAGS.l2_norm_clip}|{FLAGS.batch_size}|CPU.prof")
    print(f'Epoch {epoch} in {epoch_time:0.5f} sec')
    ...
    jax.profiler.save_device_memory_profile("ResNet-34|" + str(FLAGS.dpsgd) + ".prof")
